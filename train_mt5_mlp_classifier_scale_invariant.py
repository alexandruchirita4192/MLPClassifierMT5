...
def build_features(df: pd.DataFrame, horizon_bars: int) -> pd.DataFrame:
    ...
    # Scale-invariant ATR feature: raw ATR is in price units, so normalize it by price.
    # Keep raw atr_14 in snapshots for diagnostics / trading stop calibration, but do not feed it to the model.
    df["atr_14_pct"] = df["atr_14"] / df["close"].replace(0.0, np.nan)
...
def save_metadata(
    ...
    meta = {
        ...
        "scale_invariant_features": True,
        "raw_atr_kept_for_diagnostics": "atr_14",
...
def write_run_in_mt5(
    ...
    txt = f"""MODEL: MLPClassifier (3 classes, scale-invariant features)
SYMBOL: {args.symbol}
TIMEFRAME: {args.timeframe}
...
def main() -> None:
    ...
    onnx_path = output_dir / "ml_strategy_classifier_mlp.onnx"
